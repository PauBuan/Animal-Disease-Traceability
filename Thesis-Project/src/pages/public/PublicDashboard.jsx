...
import { tableData } from "./../../config/data"; // ← Import real data
...
export default function Dashboard() {
  ...
  const healthStatusData = tableData.map((row) => ({
    ...
    quarantined:
      row.dita.quarantined + row.pooc.quarantined + row.macabling.quarantined,
  }));
  ...
  const chartOptions = {
    ...
    scales: {
      y: {
        beginAtZero: true,
        title: { display: true, text: "Number of Movements", color: "#15803D" },
        ticks: { color: "#4B5563" },
      },
  ...
  tableData.forEach((row) => {
    ...
    if (row.animal === "Hogs") sickByDisease.ASF += totalSick;
      ...
      sickByDisease["Avian Influenza"] += totalSick;
    ...
    <div className="min-h-screen flex flex-col bg-white pt-24 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-screen-2xl mx-auto">
        ...
        <p className="text-lg text-gray-600 text-center mb-12 leading-relaxed max-w-3xl mx-auto">
          Welcome to the centralized hub for monitoring and managing animal
          disease traceability. This dashboard provides real-time insights and
          tools to ensure the health and safety of livestock across Santa Rosa
          City Laguna.
        </p>
        ...
        <section className="py-8 w-full">
          ...
          <p className="text-lg text-gray-600 text-center mb-8 leading-relaxed max-w-3xl mx-auto">
            Track the movement of animals and monitor their health status with
        ...
        <section className="py-8 bg-gray-50 w-full rounded-b-2xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            >
              ...
              <button
                onClick={() => navigate("/outbreak-stats")}
                className="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 transition w-full font-medium mt-5"
              >
                View Details
              </button>
